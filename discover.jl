...
In OHDSI, the [*SNOMED*](https://browser.ihtsdotools.org/?perspective=full&conceptId1=404684003&edition=MAIN/SNOMEDCT-US/2024-03-01&release=&languages=en) vocabulary is the primary vocabulary used for standard concepts, which provides support for [succinct groupers](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6115233/). SNOMED is not hierarchical, each clinical record may be counted under several of these groupers.

Following are high-level groupers with corresponding patient counts. This list is generated by choosing the highest-level categories for each clinical event, excluding a few that are particularly generic.
"""

# ╔═╡ 5392b439-0db1-4563-b75a-c5df8fd8e261
md"""Alternatively, in the TRDW, the *ICD10CM* vocabulary is found in the *source* concepts. This is not the standard OHDSI approach to querying diagnosis records. Following concept sets are defined with these 3 character ICD10CM categories.
"""
...
@query begin
	...
	group_by_concept(; person_threshold=10)
end
